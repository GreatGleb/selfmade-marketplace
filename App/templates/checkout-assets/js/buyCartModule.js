		...
		if (xhr.readyState == 4 && xhr.status == 200 && e.target.response) {
			if(atributesId == '') {
				...
				if(atributesId == '') {
					...
					buttonSvg.addEventListener('click', launchToggleModalBlock.bind(null, 2, 1));
				}
			}
...
function updateCountersOfProducts() {
    let allProducts = document.querySelectorAll('.checkout-product');
    let i = 1;
    
    for(let el of allProducts){
        let n = el.querySelector('.checkout-block__title-informer');
        n.innerHTML = i;
        i++;
    }
}
...
function deleteBoughtProductFromCart(container, number) {
	...
	let productDOM = document.querySelector('.checkout-product[data-product-id="' + productId + '"]');
	let liOfItProduct = productDOM.parentNode.parentNode.parentNode.querySelector('li');
	productDOM.parentNode.parentNode.remove();
	if(liOfItProduct == null) {
	    productDOM.parentNode.parentNode.parentNode.parentNode.remove();
	}
	updateCountersOfProducts();
	...
	let productCart = document.querySelector('.product-layout[data-product-id="' + productId + '"]');
	if(productCart != null) {
		let boughtAtributes = productCart.dataset.boughtAtributes;
		
		if(boughtAtributes == '') {
			let spanSvg = productCart.querySelector('.product-item-container .right-block .buttonBuyed');
			newSpanSvg = spanSvg.cloneNode(true);
			
			spanSvg.parentNode.insertBefore(newSpanSvg, spanSvg);
			spanSvg.parentNode.removeChild(spanSvg);
			
			let button = newSpanSvg.querySelector('svg');
			
			editSvgProductToEmptyCart(button);
	...
	if(allBoughtProducts.length > 0) {
		...
		if(numberProductsOnHeader != null) {
	...
	headerModule.innerHTML = " <div style=\"\
								  ...
								  \">\
									...
									<p style=\"\
										...
										cursor: pointer;\
										text-align: center;\
									\" class=\"goToBuyCart toggleModal2\">\
										Перейти в корзину\
									</p>\
								</div>";
	let buttonToCart = headerModule.querySelector('.goToBuyCart');
	buttonToCart.addEventListener('click', launchToggleModalBlock.bind(null, 2, 1));
}
...
function updateNumberBoughtProducts(container) {
	...
	if(miniCounterProductsOnHeader != null) {
...
function updateProductCost(divCounter) {
    let productId = divCounter.parentNode.parentNode.dataset.productId;
    let productDOM = document.querySelector('.checkout-product[data-product-id="' + productId + '"]');
	...
	let cost = Number(number)*Number(basePrice);
	spanPriceCount.innerHTML = cost;
	
	productDOM.dataset.productNumber = number;
	productDOM.querySelector('.js-product-quantity .checkout-product__digit').innerHTML = number;
	productDOM.dataset.productCost = cost;
	productDOM.querySelector('.js-product-amount .checkout-product__digit').innerHTML = cost + '&#8381;';
...
function addProductToBuyCart(boughtProducts, containerBoughtProducts) {
	...
	img.src = '/admin/' + imgUrl.substring(0, posImgLastSlesh) + imgUrl.substring(posImgLastSlesh);//'_150x150'
	...
	divPriceCount.innerHTML = "<span data-price=\"" + Number(boughtProducts[2]) + "\">" + Number(boughtProducts[2])*Number(boughtProducts[3]) + "</span> &#8381;";
...
function changeClassSpanSvg(buttonSvg) {
	let span = buttonSvg.parentNode;
	span.classList.remove('buttonBuy');
	span.classList.add('buttonBuyed');
}
...
function createMainContainerOrderBuyCart(containerBoughtProducts) {
	...
	divButtonContinueShopping.addEventListener('click', launchToggleModalBlock.bind(null, 2, 1));
