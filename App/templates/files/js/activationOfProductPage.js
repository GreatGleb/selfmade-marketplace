let buttonsAddToCart = document.querySelector('.svg.buttonBuy');

console.log(buttonsAddToCart);

buttonsAddToCart.addEventListener('click', addingToCartP);
...
function changeButtonToBuyed(buttonSvg) {
...
function changeButtonToBuy(buttonSvg) {
	let buttonToBuy = document.querySelector('.buttonBuy');
...
function addProducToCartDOM_Product(divProduct, atributesId) {
	let productId = divProduct.dataset.productId;
	let name = document.querySelector('.title-product h1').innerText;
	let price = document.querySelector('.product_page_price .price span').innerText;
	...
	let img = document.querySelector('.lightSlider li:first-child a img').getAttribute('src');
...
function addProducToCartP(buttonSvg, atributesId) {
	...
	xhr.addEventListener('readystatechange', function(e) {			
		if (xhr.readyState == 4 && xhr.status == 200 && e.target.response) {
			addProducToCartDOM_Product(divProduct, atributesId);
			if(atributesId == '') {
				changeButtonToBuyed();
			}
...
function chooseProductAtributeP(divProduct) {
	...
	let titleProduct = document.querySelector('.title-product h1').innerText;
	...
	let oldPrice = document.querySelector('.product_page_price .old-price');
	if(oldPrice == null) {
		spanPrice.style.color = '#3b3b3b';
	} else {
	    spanPrice.style.color = '#e74c3c';
	}
