...
abstract class Model
{
	...
	public static function findByOrderDesc($field)
    {
        $db = Db::instance();
		return $db->query(
	...
	public static function findCurrentDisconts($field, $id)
    {
        $db = Db::instance();
		return $db->query(
            'SELECT * FROM ' . static::TABLE . ' WHERE ' . $field . ' = :id AND STR_TO_DATE(dateStart, \'%Y-%m-%d\') <= CURRENT_DATE AND STR_TO_DATE(dateFinish, \'%Y-%m-%d\') >= CURRENT_DATE',
            [':id' => $id],
            static::class
        );
    }
