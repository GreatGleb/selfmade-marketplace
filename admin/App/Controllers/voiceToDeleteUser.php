...
if($_SESSION['user']['id'] !== $user->id
	...
	&& (($_SESSION['user']['isfounder'] > 0) 
		...
		if($_SESSION['user']['isfounder'] !== '1' && ($user->typeofuser == 'Admin' || ($user->iswasadmin == '1' && $days > 29))) {
			...
			if($countVoices < 1) {
				$voicefordeleteusers = new VoiceToDeleteUser;

				$voicefordeleteusers->id = $voicefordeleteusers->findMaxId() + 1;
				$voicefordeleteusers->userId = $_SESSION['user']['id'];
				$voicefordeleteusers->deleteUserId = $data;
				
				if($voicefordeleteusers->insert()) {
					echo 'Success!';
				} else {
					return false;
				}
